...
*   `GET /api/Hospital/Get-Profile`: (Hospital required) Get the logged-in hospital's profile.
*   `POST /api/Hospital/Add`: (Hospital required) Add a new hospital profile.
*   `PUT /api/Hospital/Update-profile`: (Hospital required) Update the logged-in hospital's profile.
*   `DELETE /api/Hospital/Delete-Profile`: (Hospital required) Delete the logged-in hospital's profile.
*   `POST /api/Hospital/AddService`: (Hospital required) Add a new service for the hospital.
*   `POST /api/Hospital/GetAllServices`: (Hospital required) Get all services for the logged-in hospital.
...
*   `GET /api/Hospital/GetServicesReviews/{id}`: (Hospital required) Get all reviews for a specific service.
*   `GET /api/Hospital/GetDashboardStats/{id}`: (Hospital required) Get dashboard statistics.

### Services (`/api/Services`)

*   `GET /api/Services`: Get a paged list of all available services (supports search params: keyword, category, lat, lon).
*   `GET /api/Services/search`: Alias for search.
